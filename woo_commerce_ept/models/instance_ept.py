...
class WooInstanceEpt(models.Model):
    ...
    @api.model
    def get_warehouse_for_export_stock(self):
        """
        This methos use to write stoxk export warehouse id as existing warehouse id
        @author : Nilam Kubavat on Date:10-Aug-2022
        @Task_id : 197714
        """
        instances = self.env['woo.instance.ept'].search([])
        for insatnce in instances:
            if insatnce.woo_warehouse_id:
                insatnce.write({'woo_stock_export_warehouse_ids': [(6, 0, insatnce.woo_warehouse_id.ids)]})
    ...
    is_woo_digest = fields.Boolean(string="Set Woo Commerce Digest?")
    ...
    def get_total_orders(self):
        ...
        def orders_of_current_week(order_query):
            qry = order_query + " and date(date_order) >= (select date_trunc('week', date(current_date))) " \
                                "order by date(date_order)"
            self._cr.execute(qry)
            return self._cr.dictfetchall()

        def orders_of_current_month(order_query):
            qry = order_query + " and date(date_order) >= (select date_trunc('month', date(current_date))) " \
                                "order by date(date_order)"
            self._cr.execute(qry)
            return self._cr.dictfetchall()

        def orders_of_current_year(order_query):
            qry = order_query + " and date(date_order) >= (select date_trunc('year', date(current_date))) " \
                                "order by date(date_order)"
            self._cr.execute(qry)
            return self._cr.dictfetchall()
    ...
    def woo_create_log_line(self, message):
        ...
        return common_log_line_obj.create({'model_id': model_id, 'message': message}).id

    def woo_prepare_log_book_vals(self, log_line_ids):
        """
    ...
    def woo_get_currency(self):
        ...
        if not currency_code:
            ...
            log_line_id = self.woo_create_log_line(message)
            log_line_ids.append(log_line_id)

        currency = self.woo_check_for_currency(currency_code)

        if log_line_ids:
            log_book_vals = self.woo_prepare_log_book_vals(log_line_ids)
            common_log_obj.create(log_book_vals)
        return currency
    ...
    def woo_get_system_info(self):
        ...
        if not isinstance(res, requests.models.Response):
            ...
            log_line_id.append(self.woo_create_log_line(message))
        ...
        if log_line_id:
            log_book_vals = self.woo_prepare_log_book_vals(log_line_id)
            self.env["common.log.book.ept"].create(log_book_vals)
            self._cr.commit()
        return response

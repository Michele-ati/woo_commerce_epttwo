...
class WooProductTemplateEpt(models.Model):
    ...
    def get_product_price(self, instance, variant):
        ...
        #     vals.update({'sale_price': str(sale_price)})
    ...
    def woo_update_stock(self, instance, woo_templates):
        ...
        woo_templates_ids = self.check_available_products_in_woocommerce(wc_api, instance)
        if woo_templates_ids:
            woo_templates = woo_templates.filtered(lambda template: template.id in woo_templates_ids.ids)
            ...
            if variations:
                ...
                for woo_variants in variant_batches:
                    ...
                    _logger.info('Completed Variations Batch Stock Update Process Completed with [status: %s]',
                                 res.status_code)
                    log_line = self.check_woocommerce_response(res, "Update Product stock", model_id)
    ...
    def check_woocommerce_response(self, response, process, model_id, common_log_book=False, product_template=False):
        """
        ...
        for woo_products in batches:
            ...
            if batch_update_data:
                ...
                if response.get('data') and response.get('data', {}).get('status') != 200:
                    log_line = common_log_line_obj.create({'model_id': model_id, 'message': response.get('message')})
                    log_lines.append(log_line.id)
    ...
    def woo_unpublished(self):
        ...
        if self.woo_tmpl_id:
            ...
            response = self.check_woocommerce_response(res, "Unpublish Product", model_id, woo_common_log_id)
            if not isinstance(response, (dict, list)):
                return False
            if response.get('data') and response.get('data', {}).get('status') not in [200, 201]:
                message = response.get('message')
                common_log_line_obj.woo_product_export_log_line(message, model_id, woo_common_log_id, self)
            else:
    ...
    def woo_published(self):
        ...
        if self.woo_tmpl_id:
            ...
            response = self.check_woocommerce_response(res, "Publish Product", model_id, woo_common_log_id)
            if not isinstance(response, (dict, list)):
                return False
            if response.get('data', {}) and response.get('data', {}).get('status') not in [200, 201]:
                message = response.get('message')
    ...
    def import_all_attribute_terms(self, wc_api, woo_attribute_id, woo_common_log_id, model_id, response):
        """
        This method is used for get the attribute value response as per request by page wise and return it
        @param wc_api: It contain the connection object between odoo and woo
        ...
        """
        attribute_term_data = []
        total_pages = response and response.headers.get('x-wp-totalpages', 0) or 1
        if int(total_pages) >= 2:
            for page in range(2, int(total_pages) + 1):
                ...
                data = self.check_woocommerce_response(res, "WooCommerce Attribute Terms/Values", model_id,
                                                       woo_common_log_id)
    ...
    def sync_woo_attribute_term(self, instance, woo_common_log_id, attribute_ids=False):
        ...
        for woo_attribute in woo_attributes:
            ...
            attributes_term_data = self.check_woocommerce_response(response, "WooCommerce Attributes Terms", model_id,
                                                                   woo_common_log_id)
            if not isinstance(attributes_term_data, list):
                continue

            attributes_term_data += self.import_all_attribute_terms(wc_api, woo_attribute, woo_common_log_id, model_id,
                                                                    response)
    ...
    def woo_import_all_attributes(self, wc_api, woo_common_log_id, model_id, response):
        ...
        if int(total_pages) >= 2:
            for page in range(2, int(total_pages) + 1):
                ...
                data = self.check_woocommerce_response(res, "WooCommerce Attributes", model_id, woo_common_log_id)
    ...
    def create_woo_product_queue(self, results, instance, common_log_id, import_all, template_id):
        """
    ...
    def get_products_from_woo_v1_v2_v3(self, instance, common_log_id, template_id=False, import_all=False,
        ...
        if results:
            if int(total_pages) >= 2:
                ...
                product_queue_ids = self.create_woo_product_queue(results, instance, common_log_id, import_all,
                                                                  template_id)
                if template_id:
                    return product_queue_ids
                else:
    ...
    def woo_product_convert_dates_by_timezone(self, instance, from_date, to_date):
        ...
        @author: Meera Sidapara on Date 28-Feb-2022.
        """
        if not from_date:
    ...
    def prepare_or_update_woo_product_data(self, results, instance, available_queue, common_log_id,
                                           product_data_queue_line_ids):
        """
        This method checks for existing queue lines otherwise
        @param results: Data of products.
        @param instance: Record of instance.
        @param available_queue: True if need to to check in queue lines.
        @param common_log_id: Record of log book.
        @param product_data_queue_line_ids: Queue lines to check.
        @author: Maulik Barad on Date 06-Nov-2020.
        """
        ...
        for result in results:
            ...
            if result.get('variations'):
                ...
                if isinstance(variants, str):
                    common_log_line_obj.woo_product_export_log_line(variants, model_id, common_log_id, False)
                    continue
    ...
    def process_product_response(self, results, instance, common_log_id, template_id=False, import_all=False):
        """
        ...
        total_results = self.prepare_or_update_woo_product_data(results, instance, available_queue, common_log_id,
                                                                product_data_queue_line_ids)
    ...
    def search_odoo_product_variant(self, woo_instance, product_sku, variant_id):
        ...
        if not woo_product:
            woo_product = woo_product_obj.with_context(active_test=False).search(
                [('product_id.default_code', '=', product_sku), ('woo_instance_id', '=', woo_instance.id)], limit=1)
    ...
    def woo_set_variant_sku(self, woo_instance, product_template_dict, product_template, sync_price_with_product=False):
        ...
        :param product_template_dict: It contain the product template info with variants and type is Dictionary
    ...
    def sync_woo_categ_with_product_v1_v2_v3(self, instance, woo_common_log_id, woo_categories, sync_images=True):
        """
        ...
        for woo_category in woo_categories:
            ...
            if woo_product_categ:
                ...
                obj_woo_product_categ.sync_woo_product_category(instance, woo_common_log_id, woo_product_categ,
                                                                sync_images_with_product=sync_images)
            else:
    ...
    def sync_products(self, product_data_queue_lines, woo_instance, common_log_book_id, skip_existing_products=False,
        ...
        @param : self, product_data_queue_lines, woo_instance, common_log_book_id,
        skip_existing_products=False, order_queue_line=False
        @return: True
        @author: Haresh Mori @Emipro Technologies Pvt. Ltd on date 21 August 2020.
        Task_id:165892
        @change : add meta mapping by Nilam Kubavat on 6th July 2022
        """
        common_log_line_obj = self.env["common.log.lines.ept"]
        ...
        for product_data_queue_line in product_data_queue_lines:
            ...
            if data["variations"]:
                new_woo_template = self.variation_product_sync(woo_instance, data, common_log_book_id,
                                                               product_data_queue_line, order_queue_line, woo_template,
                                                               product_queue_id, sync_category_and_tags,
    ...
    def template_attribute_process(self, woo_instance, odoo_template, variant, template_title, common_log_book_id, data,
        ...
        """
        This method use to create new attribute if customer only add the attribute value
        other wise it will create a mismatch logs.
        @param :self,woo_instance,odoo_template,variant,template_title,common_log_book_id,data,
        product_data_queue_line,order_queue_line
    ...
    def simple_product_sync(self, woo_instance, product_response, common_log_book_id, product_queue_id,
        ...
        @param :self, woo_instance, product_response, common_log_book_id, template_info, product_queue_id,
        product_data_queue_line, template_updated, skip_existing_products, order_queue_line
        ...
        if not woo_product:
            if not woo_template:
                if not odoo_template and woo_instance.auto_import_product:
                    ...
                    if woo_instance.woo_product_categ_id:
                        template_vals.update({"categ_id": woo_instance.woo_product_categ_id.id})
    ...
    def update_products_in_woo(self, instance, templates, update_price, publish, update_image,
        ...
        woo_templates_ids = self.check_available_products_in_woocommerce(wc_api, instance)
        if woo_templates_ids:
            templates = templates.filtered(lambda template: template.id in woo_templates_ids.ids)
        ...
        for templates in batches:
            ...
            if batch_update_data:
                ...
                response = self.check_woocommerce_response(res, "Update Product", model_id, common_log_id)
    ...
    def update_stock(self, instance, export_stock_from_date):
        ...
        queue_ids = False
        if woo_templates:
            # self.with_context(updated_products_in_inventory=odoo_products).woo_update_stock(instance, woo_templates)
            queue_ids = self.with_context(
    ...
    def get_product_attribute(self, template, instance, common_log_id, model_id):
        ...
        for attribute_line in template.attribute_line_ids:
            ...
            if instance.woo_attribute_type == 'select':
                attrib_data = self.export_product_attributes_in_woo(instance, common_log_id, model_id,
            ...
            if update_price:
                ...
                if instance.woo_extra_pricelist_id:
                    sale_price, sale_price_rule = instance.woo_extra_pricelist_id.get_product_price_list_rule(
                        ...
                        partner=False)
                    if sale_price_rule:
                        info.update({'sale_price': str(sale_price)})
            ...
            if template.woo_tmpl_id != variant.variant_id:
                ...
                if update_price:
                    ...
                    if instance.woo_extra_pricelist_id and sale_price_rule:
                        data.update({'sale_price': str(sale_price)})
                flag = True
    ...
    def export_products_in_woo(self, instance, woo_templates, update_price, publish, update_image, basic_detail,
        ...
        for woo_template in woo_templates:
            ...
            if woo_tmpl_id and variants:
                response_variations = self.export_woo_variants(variants, woo_tmpl_id, wc_api, woo_template, model_id)

            self.woo_update_template_variant_data(response_variations, woo_template, common_log_id, response,
                                                  woo_tmpl_id, publish)
            if instance.woo_attribute_type == 'select':
                attribute_ids = []
    ...
    def prepare_product_data(self, woo_template, publish, update_price,
        ...
        if basic_detail:
            ...
            attributes, is_variable = self.get_product_attribute(template, instance, common_log_id, model_id)
            if is_variable:
    ...
    def export_woo_variants(self, variants, woo_tmpl_id, wcapi, woo_template, common_log_id):
        ...
        for woo_variants in variant_batches:
            ...
            if variant_response.status_code not in [200, 201]:
                common_log_line_obj.woo_product_export_log_line(variant_response.content, model_id, common_log_id,
    ...
    def get_product_link(self):
        ...
        @author: Yagnik Joshi on Date 12-January-2023.
        """
        self.ensure_one()

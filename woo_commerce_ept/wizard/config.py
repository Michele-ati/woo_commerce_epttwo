...
class WooInstanceConfig(models.TransientModel):
    ...
    def prepare_val_for_instance_create(self, consumer_key, consumer_secret, host, verify_ssl, version):
        ...
        instance_vals = {'name': self.name,
                         ...
                         # "customer_so_number": self.customer_so_number
                         }
        return instance_vals
...
class ResConfigSettings(models.TransientModel):
    ...
    woo_last_completed_order_import_date = fields.Datetime(string="Last Complete Order Import Date",
                                                           help="This is the date when last complete order you have imported in "
                                                                "Odoo.\nSystem will set this date in 'From date' while import "
                                                                "order process.")
    woo_last_cancel_order_import_date = fields.Datetime(string="Import Woo Cancel Orders",
                                                        help="Last date of sync cancel orders from Woo to Odoo")
    ...
    woo_weight_uom_id = fields.Many2one("uom.uom", string="WooCommerce Weight Unit",
                                        ...
                                        help="Select Weight unit same as WooCommerce Store "
                                             "for setting proper Weight in Product.")
    ...
    woo_analytic_account_id = fields.Many2one('account.analytic.account', string='Woo Analytic Account',
                                              domain="['|', ('company_id', '=', False), "
                                                     "('company_id', '=', woo_company_id)]")
    woo_analytic_tag_ids = fields.Many2many('account.analytic.tag', 'woo_res_config_analytic_account_tag_rel',
                                            string='Woo Analytic Tags',
                                            domain="['|', ('company_id', '=', False), ('company_id', '=', woo_company_id)]")
    woo_import_products_last_date = fields.Datetime(string="Last Product Import Date",
                                                    help="Connector only imports those products which have created "
                                                         "after a given date.")
    woo_stock_export_warehouse_ids = fields.Many2many('stock.warehouse', string="Stock Export Warehouse")
    is_woo_digest = fields.Boolean(string="Send Periodic Digest?")
    show_net_profit_report = fields.Boolean(string='Net Profit Report',
                                            config_parameter="woo_commerce_ept.show_net_profit_report")
    woo_product_categ_id = fields.Many2one("product.category", string="Product Category",
                                           help="This categoty will set on new create products.")
    ...
    def onchange_woo_instance_id(self):
        ...
        if instance:
            ...
            self.woo_extra_pricelist_id = instance.woo_extra_pricelist_id.id \
                if instance.woo_extra_pricelist_id else False
            ...
            self.woo_last_cancel_order_import_date = instance.last_cancel_order_import_date
            ...
            self.woo_analytic_tag_ids = instance.woo_analytic_tag_ids.ids
            self.woo_import_products_last_date = instance.import_products_last_date
            ...
            self.woo_product_categ_id = instance.woo_product_categ_id.id \
                if instance.woo_product_categ_id else False
    ...
    def execute(self):
        ...
        if instance:
            ...
            values['woo_extra_pricelist_id'] = self.woo_extra_pricelist_id.id \
                if self.woo_extra_pricelist_id.id else False
            values[
            ...
            values['last_cancel_order_import_date'] = self.woo_last_cancel_order_import_date or False
            ...
            values["import_products_last_date"] = self.woo_import_products_last_date
            values[
            ...
            values['woo_product_categ_id'] = self.woo_product_categ_id or False
    ...
    def download_woo_commerce_net_profit_report_module(self):
        """
        This Method relocates download zip file of woo_commerce Net Profit Report module.
    ...
    def woo_save_basic_configurations(self):
        ...
        if instance:
            basic_configuration_dict = {
                ...
                'woo_product_categ_id': self.woo_product_categ_id and self.woo_product_categ_id.id or False,
            }

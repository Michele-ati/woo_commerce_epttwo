...
class WooProcessImportExport(models.TransientModel):
    ...
    def _onchange_woo_operation(self):
        ...
        if self.woo_instance_id:
            ...
            elif self.woo_instance_id.last_cancel_order_import_date and self.woo_operation == 'import_cancel_orders':
                self.orders_after_date = self.woo_instance_id.last_cancel_order_import_date
            else:
        ...
        if self.woo_operation == "import_cancel_orders":
    ...
    def execute(self):
        ...
        if self.woo_operation == "import_customer":
            ...
            sale_order_obj.sudo().import_woo_cancel_order(self.woo_instance_id, self.orders_after_date,
            ...
            action = self.env.ref("woo_commerce_ept.action_woo_export_stock_queue").sudo().read()[0]
            form_view = [(self.env.ref('woo_commerce_ept.woo_export_stock_form_view_ept').id, 'form')]
    ...
    def import_specific_product(self):
        ...
        @author : Nilam Kubavat at 27-Dec-2022
        @task ID : 212533
        """
        start = time.time()
        woo_common_log_obj = self.env["common.log.book.ept"]
        common_log_id = woo_common_log_obj.woo_create_log_book('import', self.woo_instance_id)
        ...
        if product_res_list:
            product_queues = self.env['woo.product.template.ept'].with_context(
                import_export_record=self.id).create_woo_product_queue(product_res_list,
    ...
    def get_products_from_woo(self):
        ...
        woo_instance_id.import_products_last_date = to_date.astimezone(pytz.timezone("UTC")).replace(
            tzinfo=None)
        end = time.time()
        _logger.info("Created product queues time -- %s -- seconds.", str(end - start))
        ...
        for customer_queue in split_every(101, customers):
            ...
            for customer in customer_queue:
                existing_customer_data_queue_line = woo_sync_customer_data.search(
                    [('woo_synced_data_id', '=', customer.get('id')), ('woo_instance_id', '=', self.woo_instance_id.id),

...
class WooCronConfigurationEpt(models.TransientModel):
    ...
    woo_stock_auto_export = fields.Boolean('Woo Stock Auto Update.', default=False,
                                           help="Check if you want to automatically update stock "
                                                "levels from Odoo to WooCommerce.")
    ...
    def search_active_existing_cron(self, xml_id, instance):
        ...
        """
        try:
            active_cron = self.env.ref('woo_commerce_ept.%s_%d' % (xml_id, instance.id), raise_if_not_found=False)
        except:
            active_cron = False
